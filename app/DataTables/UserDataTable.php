    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action',function($data){
                return '<a href="'.route('task.user.show',['id'=>$data->id,'name'=>str_replace(' ','_',$data->name)]).'">
                             <i title="Show" class="fa fa-eye showing"
                            data-id="'.$data->id.'" style="cursor: pointer;color:blue;font-size:17px"></i>
                        </a>
                               <a href="'.route('task.user.edit',['id'=>$data->id,'name'=>str_replace(' ','_',$data->name)]).'">
                                <i title="Edit"
                                class="fa fa-pencil edit" data-id="'.$data->id.'" data-route="'.route('task.user.edit',$data->id).'"
                                ...
                                data-route="'.route('task.user.delete',$data->id).'" style="cursor: pointer;color:#ff0000;font-size:17px"></i>';
